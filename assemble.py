...
def encode_addi(rd, rs1, value):
    return ((value & 0xFFF) << 20) | (rs1 << 15) | (0 << 12) | (rd << 7) | 0x13

def encode_lui(rd, value):
...
def encode_li(rd, value):
    ...
    if lower:
        words.append(encode_addi(rd, rd, lower))
    return words
...
def assemble_line(line, labels, pc):
    ...
    if op == 'li':
        return encode_li(reg(a[0]), imm_raw(a[1]))
    if op == 'ble':
...
def instruction_count(line):
    ...
    if parts[0].lower() == 'li':
        return len(encode_li(reg(parts[1]), imm_raw(parts[2])))
    return 1
    ...
    for ln in lines:
        ...
        if r is not None:
            ...
            if not isinstance(r, tuple) or r[0] != 'directive':
                pc += 4 * (len(r) if isinstance(r, list) else 1)
